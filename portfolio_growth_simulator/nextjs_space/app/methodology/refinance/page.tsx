  ...
  return (
    <div className="min-h-screen bg-background px-4 py-6 md:px-8 md:py-10">
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <header className="space-y-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary/10 text-primary"><Calculator className="h-5 w-5" /></div><h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Refinance Methodology</h1><p className="text-muted-foreground">The refinance tool compares two deterministic fixed-rate amortization schedules from the same starting month.</p></header>

        <Card><CardHeader className="pb-3"><h2 className="flex items-center gap-2 text-lg font-semibold"><Scale className="h-5 w-5 text-primary" />Remaining-cost comparison</h2></CardHeader><CardContent className="space-y-3 text-sm leading-relaxed text-muted-foreground"><p>The current loan uses the entered remaining balance, APR, and remaining term. The replacement loan uses the proposed APR and term. Closing costs are added either to the new principal when financed or to the refinance total when paid upfront.</p><p><strong className="text-foreground">Lifetime savings</strong> equals current remaining scheduled payments minus the refinance&apos;s scheduled payments and closing costs. A negative result means the refinance costs more over the modeled remaining life.</p></CardContent></Card>

        <Card><CardHeader className="pb-3"><h2 className="text-lg font-semibold">Payment and break-even</h2></CardHeader><CardContent className="space-y-3 text-sm leading-relaxed text-muted-foreground"><p>Monthly payment savings is the current required payment minus the new required payment. The displayed break-even estimate is closing costs divided by positive monthly payment savings, rounded up to a whole month.</p><p>This is deliberately a simple cash-flow break-even measure. It does not discount future cash flows, model home equity, tax deductions, opportunity cost, prepayment penalties, or a future sale before the break-even date.</p></CardContent></Card>
