  ...
  return (
    <div className="min-h-screen bg-background px-4 py-6 md:px-8 md:py-10">
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <header className="space-y-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary/10 text-primary"><Scale className="h-5 w-5" /></div><h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Invest vs. Debt Methodology</h1><p className="text-muted-foreground">The comparison keeps the monthly household cash commitment equal under both strategies, then applies the same market path to both investment accounts.</p></header>

        <Card><CardHeader className="pb-3"><h2 className="text-lg font-semibold">Fair cash-flow comparison</h2></CardHeader><CardContent className="space-y-3 text-sm leading-relaxed text-muted-foreground"><p><strong className="text-foreground">Invest first:</strong> make the required loan payment and invest the entered extra monthly cash. <strong className="text-foreground">Debt first:</strong> apply that extra cash to principal until the loan is paid off, then invest the entire former loan-payment budget for the rest of the original term.</p><p>Any unused amount in the final payoff month is invested so neither strategy silently loses cash. Contributions are added at the end of each modeled month after that month&apos;s market return, matching the site&apos;s Growth timing convention.</p></CardContent></Card>

        <Card><CardHeader className="pb-3"><h2 className="flex items-center gap-2 text-lg font-semibold"><TrendingUp className="h-5 w-5 text-primary" />Market scenarios</h2></CardHeader><CardContent className="space-y-3 text-sm leading-relaxed text-muted-foreground"><p>The randomized comparison uses seeded monthly lognormal returns. The entered expected annual return determines the geometric drift and the entered annual volatility determines the monthly dispersion. The same monthly return path is applied to both strategies inside each scenario so the comparison is not distorted by different random markets.</p><p>The probability shown is the percentage of scenarios in which the invest-first account finishes with a larger value at the end of the original loan term. Seeded scenarios are reproducible estimates, not forecasts or guarantees.</p></CardContent></Card>

        <Card><CardHeader className="pb-3"><h2 className="text-lg font-semibold">Debt payoff benefit</h2></CardHeader><CardContent className="space-y-3 text-sm leading-relaxed text-muted-foreground"><p>Accelerating a fixed-rate loan reduces contractual interest with no market volatility. The tool reports the modeled interest saved and earlier payoff from sending the extra monthly cash to principal first.</p><p>This should not be interpreted as a universal after-tax investment hurdle rate. Mortgage-interest deductions, investment taxes, account type, employer matches, liquidity needs, and individual risk tolerance can materially change the real-world decision.</p></CardContent></Card>
